...
if [[ -z "${AIDBOX_LICENSE_KEY:-}" ]]; then
  for env_file in "$ROOT_DIR/.env" "$(dirname "$ROOT_DIR")/.env"; do
...
if [[ -z "${AIDBOX_LICENSE_KEY:-}" ]]; then
  echo "ERROR: AIDBOX_LICENSE_KEY not found. Export it or add to .env"
  exit 1
fi
...
if [[ "$SKIP_HTTPYAC" == "false" ]] && command -v httpyac &>/dev/null; then
  ...
  # Substitute hardcoded URL and admin credentials for the test Aidbox.
  # Profile tests use base64(admin:password) — replace with literal basic:secret
