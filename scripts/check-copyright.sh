...
#
# Exit 0: no violations found (or --warn-only). Exit 1: violations found.
#
...
# --- Default strict scan scope ---
# The blocklist contains short dental terms that are valid in other IG content.
# By default, the blocking guard protects the catalog CodeSystem sources that
# previously carried verbatim BEMA/BEL-II/beb97 display strings. Passing an
...
# For implicit hook/CI scans, restrict to the catalog sources where reintroduced
# verbatim displays are the actual publication risk. Explicit '-- <files>' calls
# intentionally bypass this narrowing.
if [ "${#EXPLICIT_FILES[@]}" -eq 0 ]; then
  STRICT_FILES_TO_SCAN=()
  for f in "${FILES_TO_SCAN[@]}"; do
    for strict in "${DEFAULT_STRICT_FILES[@]}"; do
      if [ "$f" = "$strict" ]; then
        STRICT_FILES_TO_SCAN+=("$f")
        break
      fi
    done
  done
  FILES_TO_SCAN=("${STRICT_FILES_TO_SCAN[@]}")
fi
...
for match in "${RAW_MATCHES[@]}"; do
  ...
  # Section comments can legitimately use generic grouping labels that overlap
  # with short blocklist patterns. The protected risk is verbatim display text
  # in resource content, not explanatory comments.
