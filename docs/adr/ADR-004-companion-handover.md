...
Erst Spike. Falls Polaris/MIRA EBZ-EDI-Format überhaupt produzieren/konsumieren → CSes + Bundle/MessageHeader-Bindings. Wenn FHIR-REST das EBZ-EDI komplett ablöst → keine fhir-dental-de-Anpassung nötig.

### Cross-Repo: `polaris-3quq` und `mira-s5xoh`

Auch wenn diese in Polaris/mira gelistet sind, betreffen sie fhir-dental-de indirekt:

- **`polaris-3quq`** — Polaris-Rebinding aller heute gegen cognovis-Vorschlag-URL persistierter Daten. Bei produktiver Migration auf KZBV-DPF-Namespace muss fhir-dental-de keine Code-Änderung machen, aber neue Validierungs-Test-Cases helfen den Konsumenten.
- **`mira-s5xoh`** — Audit der `mira.cognovis.de/fhir/CodeSystem/ze-*`-URLs. Wenn die existieren und produktive Daten dagegen liegen, ist das ein weiterer Cleanup-Pfad. Falls die URLs nicht existieren → Polaris-Docs korrigieren.
...
- **Charly-Befund-Decoding Spike**: `~/code/polaris/packages/pvs-charly/docs/zahnstatus-decoding.md`
...
| polaris | polaris-3quq | ○ open P2 | Polaris-Rebinding |
| mira | mira-s5xoh | ○ open P3 | Mira URL-Audit |
